-- RBAC Migration: Upgrade to full role-based access control (FIXED VERSION)
-- This migration adds the new RBAC system while preserving existing data
-- Handles the case where role column is an enum type

-- Step 1: Check current role column type and handle accordingly
DO $$
DECLARE
...
-- Step 2: Update the profiles table to support the new role system
...
-- Step 3: Create the permissions table for fine-grained access control
...
-- Step 4: Create audit_logs table for RBAC event tracking
...
-- Step 5: Insert the RBAC permission matrix
...
-- Step 6: Update the handle_new_user function to use the new role system
...
-- Step 9: Create a function to check if RBAC is enabled
