-- Fix Create User button with unique ID generation
-- This script creates users with proper unique ID handling
...
-- Step 2: Drop and recreate the create_username_user function with unique ID generation
...
SELECT 'auth.users' as table_name, id, email, encrypted_password FROM auth.users WHERE email = 'testuser222@local.com'
UNION ALL
SELECT 'profiles' as table_name, id::text, email, password_hash FROM public.profiles WHERE username = 'testuser222';
...
DELETE FROM public.profiles WHERE username = 'testuser222';
DELETE FROM auth.users WHERE email = 'testuser222@local.com';
