-- Fix Create User button with direct insert approach
-- This script bypasses the problematic function and uses direct inserts
...
-- Step 3: Create a simple function that just inserts into profiles table
...
-- Step 4: Test direct insert first
INSERT INTO public.profiles (
  id,
  user_id,
  username,
...
-- Step 5: Check if the direct insert worked
...
-- Step 7: Check if the function worked
SELECT id, username, full_name, role, created_at 
FROM public.profiles 
WHERE username = 'testuser000';

-- Step 8: Test the profiles query that was failing
SELECT id, username, full_name, role, created_at 
FROM public.profiles 
...
-- Step 9: Clean up test users
DELETE FROM public.profiles WHERE username IN ('testuser999', 'testuser000');
