-- Fix Create User button with timestamp-based UUID generation
-- This script uses timestamp to ensure unique UUIDs
...
-- Step 3: Create a function that uses timestamp-based UUID generation
...
BEGIN
  ...
  -- Generate a unique UUID using timestamp and random
  timestamp_str := extract(epoch from now())::text;
  random_str := random()::text;
  new_user_id := ('00000000-0000-0000-0000-' || lpad(replace(timestamp_str, '.', '') || replace(random_str, '.', ''), 12, '0'))::uuid;
...
SELECT 'auth.users' as table_name, id, email, encrypted_password FROM auth.users WHERE email = 'testuser444@local.com'
UNION ALL
SELECT 'profiles' as table_name, id::text, email, password_hash FROM public.profiles WHERE username = 'testuser444';
...
DELETE FROM public.profiles WHERE username = 'testuser444';
DELETE FROM auth.users WHERE email = 'testuser444@local.com';
