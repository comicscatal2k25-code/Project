-- RBAC Migration: Ultimate version that handles ALL RLS policy dependencies
-- This migration removes ALL policies from ALL tables before altering the role column

-- Step 1: Disable RLS on ALL tables that might have policies
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.comics DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.comic_images DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.comic_variants DISABLE ROW LEVEL SECURITY;

-- Step 2: Drop ALL existing policies from ALL tables in the public schema
DO $$
DECLARE
    pol RECORD;
BEGIN
...
-- Step 11: Re-enable RLS and recreate basic policies for profiles table
...
-- Step 12: Re-enable RLS and recreate basic policies for comics table
...
-- Step 13: Re-enable RLS and recreate basic policies for comic_images table
-- Check if the table exists and has the expected columns
DO $$
BEGIN
    -- Check if comic_images table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'comic_images' AND table_schema = 'public') THEN
        ALTER TABLE public.comic_images ENABLE ROW LEVEL SECURITY;
        
        -- Check if user_id column exists
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'comic_images' AND column_name = 'user_id' AND table_schema = 'public') THEN
            -- Basic comic_images policies (users can manage their own comic images)
            CREATE POLICY "comic_images_select_own" ON public.comic_images
              FOR SELECT USING (auth.uid() = user_id);

            CREATE POLICY "comic_images_insert_own" ON public.comic_images
              FOR INSERT WITH CHECK (auth.uid() = user_id);

            CREATE POLICY "comic_images_update_own" ON public.comic_images
              FOR UPDATE USING (auth.uid() = user_id);

            CREATE POLICY "comic_images_delete_own" ON public.comic_images
              FOR DELETE USING (auth.uid() = user_id);
        ELSE
            -- If no user_id column, create basic policies without user filtering
            CREATE POLICY "comic_images_select_all" ON public.comic_images
              FOR SELECT USING (true);

            CREATE POLICY "comic_images_insert_all" ON public.comic_images
              FOR INSERT WITH CHECK (true);

            CREATE POLICY "comic_images_update_all" ON public.comic_images
              FOR UPDATE USING (true);

            CREATE POLICY "comic_images_delete_all" ON public.comic_images
              FOR DELETE USING (true);
        END IF;
    END IF;
END $$;

-- Step 14: Re-enable RLS and recreate basic policies for comic_variants table
-- Check if the table exists and has the expected columns
DO $$
BEGIN
    -- Check if comic_variants table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'comic_variants' AND table_schema = 'public') THEN
        ALTER TABLE public.comic_variants ENABLE ROW LEVEL SECURITY;
        
        -- Check if user_id column exists
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'comic_variants' AND column_name = 'user_id' AND table_schema = 'public') THEN
            -- Basic comic_variants policies (users can manage their own comic variants)
            CREATE POLICY "comic_variants_select_own" ON public.comic_variants
              FOR SELECT USING (auth.uid() = user_id);

            CREATE POLICY "comic_variants_insert_own" ON public.comic_variants
              FOR INSERT WITH CHECK (auth.uid() = user_id);

            CREATE POLICY "comic_variants_update_own" ON public.comic_variants
              FOR UPDATE USING (auth.uid() = user_id);

            CREATE POLICY "comic_variants_delete_own" ON public.comic_variants
              FOR DELETE USING (auth.uid() = user_id);
        ELSE
            -- If no user_id column, create basic policies without user filtering
            CREATE POLICY "comic_variants_select_all" ON public.comic_variants
              FOR SELECT USING (true);

            CREATE POLICY "comic_variants_insert_all" ON public.comic_variants
              FOR INSERT WITH CHECK (true);

            CREATE POLICY "comic_variants_update_all" ON public.comic_variants
              FOR UPDATE USING (true);

            CREATE POLICY "comic_variants_delete_all" ON public.comic_variants
              FOR DELETE USING (true);
        END IF;
    END IF;
END $$;
...
-- Step 16: Create function to log audit events
...
-- Step 17: Create a function to check if RBAC is enabled
