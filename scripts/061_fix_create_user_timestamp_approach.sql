-- Fix Create User button with timestamp-based approach
-- This script uses timestamp to ensure unique IDs
...
-- Step 2: Drop and recreate the create_username_user function with timestamp-based approach
...
BEGIN
  ...
  -- Generate a unique UUID using timestamp
  timestamp_str := extract(epoch from now())::text || '-' || random()::text;
  new_user_id := ('00000000-0000-0000-0000-' || lpad(replace(timestamp_str, '.', ''), 12, '0'))::uuid;
